...
function addcustommenu() {
  ...
  $.ajax({
    data: {
      labelmenu: $('#custom-wmenu-item-name').val(),
      linkmenu: $('#custom-wmenu-item-url').val(),
      rolemenu: $('#custom-wmenu-item-role').val(),
      idmenu: $('#idmenu').val()
    },
...
function updateitem(id = 0) {
  if (id) {
    ...
    var arr_data = [];
    $('.wmenu-item-settings').each(function(k, v) {
      var id = $(this)
        .find('.edit-wmenu-item-id')
        .val();
      var label = $(this)
        .find('.edit-wmenu-item-title')
        .val();
      var clases = $(this)
        .find('.edit-wmenu-item-classes')
        .val();
      var url = $(this)
        .find('.edit-wmenu-item-url')
        .val();
      var role = $(this)
        .find('.edit-wmenu-item-role')
        .val();
      arr_data.push({
        id: id,
        label: label,
